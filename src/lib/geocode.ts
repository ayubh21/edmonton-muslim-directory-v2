...
export class Geocode {
  ...
  async getFromPlaceId(placeId: string) {
    const result = await fetch(
      `https://places.googleapis.com/v1/places/${placeId}?fields=id,displayName&key=${this.apiKey}`,
    );
    const data = await result.json();
    return data;
  }
...
export const geocode = new Geocode(process.env.GCLOUD_GEOCODE_API_KEY!);
